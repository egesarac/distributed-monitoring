...
%	The interpretations of the subformulas can be computed by a dynamic programming algorithm in the sprit of~\cite{Havelund2002}, using the standard recursive definition of the until operator. % $p \until q = q \lor (p \land \LTLnext (p \until q))$
	...
	\greennote[inline]{Idea: Add the 2nd preprocessing step into the semantics by defining two functions $\rho_{\text{lo}}$ and $\rho_{\text{hi}}$ that maps an event to its lower and upper bounds of uncertainty.}
	\vspace{1em}
	...
	where $i \leq k \leq n_p$ satisfies $t^p_{j + 1} - t^p_j < \delta + 2\varepsilon$ for all $i \leq j \leq k - 1$.
