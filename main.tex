...
%		\item The monitor runs on a process which does not own any propositions. \rednote{to confirm}
		...
		\item The values of the atomic propositions stay the same for at least $\delta > 0$ time units after each change with respect to the local clock of the monitor. The value of the greatest lower bound on $\delta$ is known by the monitor.
	...
	\rednote{TODO: This approach is too coarse: (i) it does not take into account that the clock skew is bounded for each process pair, and (ii) it fails to capture the truth values of close-enough events when $\delta < 2\varepsilon$.}
