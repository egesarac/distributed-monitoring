...
def prog_not_always_implies_eventually_timed(eps, segCount, data_0, data_1, a, b):
    ...
    while entryFound:
        ...
        s.add(
            ForAll(u, Implies(And(u >= timestamps_0[0], u <= timestamps_0[-1], z3Interpolate(sig0, c0(u)) == 1),
                              And(v >= z3min(u + a, timestamps_0[-1]), v < z3min(u + b, timestamps_0[-1] + 1), z3Interpolate(sig1, c1(v)) == 1)
                    )
            )
        )
...
def prog_always_implies_eventually_timed(eps, segCount, data_0, data_1, a, b):
    ...
    while entryFound:
        ...
        s.add(
            And(u >= timestamps_0[0], u <= timestamps_0[-1], z3Interpolate(sig0, c0(u)) == 1,
                ForAll(v, Implies(And(v >= z3min(u + a, timestamps_1[-1]), v < z3min(u + b, timestamps_0[-1] + 1)), z3Interpolate(sig1, c1(v)) == 0)))
        )
...
def prog_always_conjunction_timed(eps, segCount, data_0, data_1, a, b):
...
def prog_always_disjunction_timed(eps, segCount, data_0, data_1, a, b):
...
def prog_eventually_conjunction_timed(eps, segCount, data_0, data_1, a, b):
...
def prog_eventually_disjunction_timed(eps, segCount, data_0, data_1, a, b):
