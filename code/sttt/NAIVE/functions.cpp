...
template <std::size_t N>
vector<vector<bitset<N>>> bitsetAlwaysPast(vector<vector<bitset<N>>> v1, int s = 0, int e = -1, bool b0 = false, bool b1 = true)
// vector<vector<bitset<N>>> bitsetAlwaysPast(const vector<vector<bitset<N>>> &v1, bool b0 = false, bool b1 = true)
{
    if (e == -1) {
        e = v1.size();
    }
...
template <std::size_t N>
vector<vector<bitset<N>>> bitsetEventuallyPast(const vector<vector<bitset<N>>> &v1, int s = 0, int e = -1, bool b0 = true, bool b1 = false)
{
    if (e == -1) {
        e = v1.size();
    }
...
template <std::size_t N>
vector<vector<bitset<N>>> bitsetBoundedAlwaysPast(vector<vector<bitset<N>>> v1, vector<long long> segmentation, const long long &a, const long long &b, const bool &leftClosed, const bool &rightClosed, int s = 0, int e = -1)  // bool b0 = true, bool b1 = false
{
    if (e == -1) {
        e = v1.size();
    }
    ...
    for (int i = s; i < e; i++)
    {
        vector<vector<bitset<N>>> temp = getBackwardProfiles(v1, segmentation, segmentation[i], segmentation[i + 1], a, b, leftClosed, rightClosed);

        for (int k = temp.size() - 1; k >= 0; k--)
        {
            vector<vector<bitset<N>>> tempPerSegment;
            tempPerSegment.push_back(temp[k]);
            tempPerSegment = bitsetAlwaysPast(tempPerSegment);
...
template <std::size_t N>
vector<vector<bitset<N>>> bitsetBoundedEventuallyPast(vector<vector<bitset<N>>> v1, vector<long long> segmentation, const long long &a, const long long &b, const bool &leftClosed, const bool &rightClosed, int s = 0, int e = -1)  // bool b0 = true, bool b1 = false
{
    if (e == -1) {
        e = v1.size();
    }
    ...
    for (int i = s; i < e; i++)
    {
        ...
        for (int k = temp.size() - 1; k >= 0; k--)
        {
            vector<vector<bitset<N>>> tempPerSegment;
            tempPerSegment.push_back(temp[k]);
            tempPerSegment = bitsetEventuallyPast(tempPerSegment);
            temp[k] = tempPerSegment[0];
        }
...
template <std::size_t N>
vector<vector<bitset<N>>> bitsetSinceNonStrict(vector<vector<bitset<N>>> v1, vector<vector<bitset<N>>> v2, int s = 0, int e = -1, bool b0 = true, bool b1 = false)
{
    if (e == -1) {
        e = v1.size();
    }
    ...
    vector<vector<bitset<N>>> vv = bitsetUntilNonStrict(v1, v2, b0, b1, ee, ss);
    reverse(vv.begin(), vv.end());
    for (auto &e : vv) {
        bitset<N> zero = e[0];
        bitset<N> one = e[1];
...
template <std::size_t N>
vector<vector<bitset<N>>> bitsetDisjunction(vector<vector<bitset<N>>> v1, vector<vector<bitset<N>>> v2)
{
    return bitsetNegation(bitsetConjunction(bitsetNegation(v1), bitsetNegation(v2)));
}
...
vector<vector<vector<long long>>> computeUncertaintyIntervals(const vector<vector<pair<long long, double>>> &signals, const int &eps, const int &del)
{
    ...
    for (int i = 0; i < signals.size(); i++)
        {
            int gi = signals[i].size(); // UPDATE: NUMBER OF EDGES IN THE RELEVANT INTERVAL
            uncertainties[i].resize(gi);

            uncertainties[i][0].push_back(-del); // UPDATE: START - DEL
            for (int j = 1; j < gi; j++)
            {
                // if (signals[i][j].second != signals[i][j - 1].second)
                // {
                    long long tj = signals[i][j].first;
                    long long low = max(uncertainties[i][j - 1][0] + del, tj - eps); // UPDATE: 
                    uncertainties[i][j].push_back(low);                   
                // }

            }

            uncertainties[i][gi - 1].push_back(signals[i][gi - 1].first + eps); // UPDATE: ???
...
// vector<vector<vector<long long>>> computeUncertaintyIntervals(const vector<vector<pair<long long, double>>> &signals, const int &eps, const int &del, int start, int end)
// {
//     vector<vector<vector<long long>>> uncertainties(signals.size());
...
//             uncertainties[i][0].push_back(start - del); // UPDATE: START - DEL
//             for (int j = 1; j < gi; j++)
//             {
//                 // if (signals[i][j].second != signals[i][j - 1].second)
//                 // {
...
vector<long long> computeCanonicalSegmentation(const vector<vector<pair<long long, double>>> &signals, const vector<vector<vector<long long>>> &uncertainties, const int &d, const int &end = 0, const int &start = 0)
{
    set<long long> segmentation_temp;

    segmentation_temp.insert(0);
    segmentation_temp.insert(end);
    segmentation_temp.insert(start);
