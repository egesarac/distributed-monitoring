...
int main()
{
    ...
    ofstream results("onlineNaive_response01.txt");
            ...
            for (auto CHUNK_SZ : CHUNK)
            {
                ...
                for (int c = 0; c < 100; ++c)
                {
                    ...
                    while (processed < d)
                    {
                        ...
                        auto test = bitsetAlwaysPast(bitsetDisjunction(bitsetNegation(aps[0]), bitsetBoundedEventuallyPast(aps[1], segmentation, 0, 1000, true, false, 0, processEnd)), 0, processEnd);
