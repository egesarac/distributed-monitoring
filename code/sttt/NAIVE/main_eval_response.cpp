...
int main()
{
    ...
    ofstream results("onlineNaive_response.txt");
            ...
            for (auto CHUNK_SZ : CHUNK)
            {
                ...
                for (int c = 0; c < 100; ++c)
                {
                    ...
                    while (processed < d)
                    {
                        ...
                        auto test = bitsetAlwaysPast(bitsetDisjunction(bitsetNegation(aps[0]), bitsetEventuallyPast(aps[1], 0, processEnd)), 0, processEnd);
