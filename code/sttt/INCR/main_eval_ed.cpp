...
int main()
{
    ...
    ofstream results("onlineIncr_ed.txt");
        ...
        for (auto eps : EPS)
        {
            for (auto CHUNK_SZ : CHUNK)
            {
                ...
                for (int c = 0; c < 100; ++c)
                {
                    ...
                    while (processed < d)
                    {
                        const int readStart = max<int>(0, lastSegmentEndPointFromPrevPortion - 2 * eps) / 1000;
                        ...
                        auto test = bitsetEventuallyPast(bitsetDisjunction(aps[0], aps[1]), processStart, processEnd, lastBit0, lastBit1);
