...
int main()
{
    ...
    ofstream results("onlineIncr_response02.txt");
        ...
        for (auto eps : EPS)
        {
            for (auto CHUNK_SZ : CHUNK)
            {
                ...
                for (int c = 0; c < 100; ++c)
                {
                    ...
                    while (processed < d)
                    {
                        const int readStart = max<int>(0, lastSegmentEndPointFromPrevPortion - 2 * eps - 2000) / 1000;
                        ...
                        auto testsub = bitsetBoundedEventuallyPast(aps[1], segmentation, 0, 2000, true, false, processStart, processEnd);
