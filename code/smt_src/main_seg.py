            ...
            for delt in (1, 2, 4, 8):
                ...
                for numEdges1 in NUMEDGES:
                    ...
                    for numEdges2 in NUMEDGES:
                        ...
                        line = "U: " + str(d) + " " + str(eps) + " " + str(delt) + " " + str(numEdges1) + " " + str(numEdges2)
                        seg = 1
                        while seg <= d:
                            results = open("results_u_smt_seg.txt", "a")
                            total_time = 0
                            for i in range(repeat):
                                start = time.time()
                                prog_always_conjunction(2*eps, seg, data_1, data_2)
                                end = time.time()
                                total_time += end - start
                            line = line + " " + str(total_time / repeat)
                            seg = seg * 2
                        print(line)
                        results.write(line + "\n")
                        results.close()
