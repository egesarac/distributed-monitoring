...
int main()
{
    ...
    string filename = "results_TESTnewhalf.txt";
        ...
        for (auto eps : EPS)
        {
            if (d < eps)
            {
                ...
                for (int c = 0; c < 100; c++)
                {
                    ...
                    signalsReal[2] = getData("dataNew/32_" + to_string(c) + ".txt", d / 1000);
                    signalsReal[3] = getData("dataNew/32_" + to_string(c + 100) + ".txt", d / 1000);
                    ...
                    for (int rep = 0; rep < REP; rep++)
                    {
                        ...
                        // always(x1 or x2)
                        test = bitsetAlways(bitsetNegation(bitsetConjunction(bitsetConjunction(bitsetNegation(aps[0]), bitsetNegation(aps[1])), bitsetConjunction(bitsetNegation(aps[2]), bitsetNegation(aps[3])))));
