...
def prog_always_conjunction(eps, segCount, data_0, data_1):
    ...
    # edges0 = []
    # edges0.append([0.0, False])
    # for k in range(1, int(data_0[-1][0])):
    #     if data_0[k][1] != data_0[k-1][1]:
    #         edges0.append([data_0[k][0], True])
    #     else:
    #         edges0.append([data_0[k][0], False])          
    ...
    # edges1 = []
    # edges1.append([0, False])
    # for k in range(1, int(data_1[-1][0])):
    #     if data_1[k][1] != data_1[k-1][1]:
    #         edges1.append([data_1[k][0], True])
    #     else:
    #         edges1.append([data_1[k][0], False])         
    ...
    while entryFound:
        ...
        s.add(
            And(
                [
                    # And(c0(i) >= 0, c0(i) <= data_0[-2][0])
                    Or([c0(i) == (min(timestamps0[-1]//pad, (max(0, ((i - peps) + j)//pad)))) for j in range(1, 2 * int(peps))])
        ...
        s.add(
            And(
                [
                    Or([c0(min(timestamps0[-1], max(0, i * pad - peps + j))) == i for j in range(2 * int(peps) + pad)])
        ...
        s.add(
            And(
                [
                    # And(c1(i) >= 0, c1(i) <= data_1[-2][0])
                    Or([c1(i) == (min(timestamps1[-1]//pad, (max(0, ((i - peps) + j)//pad)))) for j in range(1, 2 * int(peps))])
        ...
        s.add(
            And(
                [
                    Or([c1(min(timestamps1[-1], max(0, i * pad - peps + j))) == i for j in range(2 * int(peps) + pad)])
        ...
        s.add(
            And(
                [
                    Implies(
                        And(
                            Or([And(c0(i) == edges0[j], c0(i) != c0(i - 1)) for j in range(len(edges0))]),
                            Or([And(c1(i) == edges1[j], c1(i) != c1(i - 1)) for j in range(len(edges1))])
                        ),
                        And(c0(i) - c1(i) < eps, c0(i) - c1(i) > -eps)
                    )
                    for i in range(timestamps1[0], timestamps1[-1] + 1)
    ...
    while entryFound:
        ...
        s.add(
            And(
                [
                    Or([c0(min(timestamps0[-1], max(0, i * pad - peps + j))) == i for j in range(2 * int(peps) + pad)])
                    for i in range(segmentUpperBound)
                    # for i in range(int(data_0[-1][0]))
                ]
            )
        )
        ...
        s.add(
            And(
                [
                    Implies(
                        And(
                            Or([And(c0(i) == var_0[j], c0(i) != c0(i - 1)) for j in range(1, len(var_0) - 1)]),
                            Or([And(c1(i) == var_1[j], c1(i) != c1(i - 1)) for j in range(1, len(var_1) - 1)])
                        ),
                        And(c0(i) - c1(i) < eps, c0(i) - c1(i) > -eps)
                    )
                    for i in range(timestamps1[0], timestamps1[-1] + 1)
        ...
        #     And(
        #         [
        #             # TODO: this works fine for d=4, id=13, but it breaks d=4, id=2 on the negation (eventually conjunction) (and vice versa)
        #             And(
        #                 Implies(i % pad > 0, And(c0(i) - c1(i) <= eps, c0(i) - c1(i) >= -eps)),
        #                 Implies(i % pad == 0, And(c0(i) - c1(i) < eps, c0(i) - c1(i) > -eps))
        #                 )
        #             for i in range(timestamps0[0], timestamps0[-1] + 1)
        #         ]
