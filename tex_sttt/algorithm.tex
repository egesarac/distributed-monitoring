...
We describe an algorithm, given a prefix $(S,{\hb})$ of a distributed signal, to compute $[(S,{\hb}) \models \varphi]_+$.

\rednote[inline]{for online monitoring, we don't know when the signal ends so we should be careful. lead into naive algo.}

\rednote[inline]{remark about correctness of naive}

\rednote[inline]{incremental, correctness of incremental against naive}

\rednote[inline]{proof idea}


\begin{theorem} \label{cl:algoOnline}
	For every distributed signal $(S,{\hb})$ and past STL formula $\varphi$ we have $\llbracket (S, {\hb}) \models \varphi \rrbracket_{\textit{naive}} = \llbracket (S, {\hb}) \models \varphi \rrbracket_{\textit{incr}}$.
	\rednote[inline]{we can prove something stronger: for each chunk, the satisfaction value expressions in that chunk are the same. define the notation accordingly. including sampling period for incr.}
\end{theorem}
