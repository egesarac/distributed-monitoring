...
\dejan{The operation $\otimes$ shall be associative. Why not define $u_1 \otimes u_2$ instead of $u_1 \otimes \ldots \otimes u_m$? It should simplify the definition. I propose (modulo $k$ which I think is not correct in the below def):
$$
u_1 \otimes u_2 = \{ \destutter(v_1, v_2)~|~v_i \in \stutter_k(u_i), \text{ where } k = |u_1| + |u_2|\}
$$}
...
\dejan{I suggest adding a concrete example that illustrates where the overapproximation comes from.}
