...
\ege{$a'$ is the limit variable -- maybe swapping $a'$ and $a$ helps?}
...
\ege{I think we don't need it since we only focus on piecewise-constant signals.}
\end{definition}
...
\borzoo{Another way is $|c(i) - c(j)| < \epsilon$}
...
\dejan{$t + 2\varepsilon \leq t'$, if $t$ and $t'$ are exactly separated by $2\varepsilon$, they are 
still totally ordered, because when projected to the global time, the first event happens in the worst 
case in the left limit of $t+\varepsilon$ and the second event in its right limit.} \borzoo{Let's discuss 
this.}
	\end{itemize}
\end{definition}
...
\borzoo{Let's use consistent notation: $E$ for events.}
\ege{I use $E$ for edges (which we refer to often) and $V$ for events (which only appear in preliminaries).}
...
function. \dejan{$f$ is already used for the consistent cut flow. I would 
suggest that we change the symbol for CCF.}
\borzoo{yes I agree. I think we used small ccf for that.} \ege{Done.}
...
\borzoo{disjunction should go first ;)}
