...
\dejan{why do we need the signal to be left-limited?}
\end{definition}
...
We assume that signals have \emph{bounded variability} with respect to the global clock: for each signal $x_i$ and every pair $(t, x_i(t)), (t', x_i(t')) \in E_i$ of edges, we have $|c_j^{-1}(t) - c_j^{-1}(t')| \geq \delta$, where $A_j = \pi(x_i)$ is the agent that produces $x_i$ and $\delta \in \R_{\geq 0}$ is the bounded variability constant.
...
\begin{definition} \label{defn:hb}
	...
	\begin{itemize}
		...
		\item Every pair of events whose timestamps are at least $2 \varepsilon$ apart is totally ordered, i.e., for all $1 \leq i,j \leq n$ and all $(t, x_i(t)) \in V_i$ and $(t', x_j(t')) \in V_j$, if $t + 2\varepsilon \leq t'$ then $(t, x_i(t)) \hb (t', x_j(t'))$. \ege{To be confirmed: $t + 2\varepsilon \leq t'$ or $t + 2\varepsilon < t'$?} 
\dejan{$t + 2\varepsilon \leq t'$, if $t$ and $t'$ are exactly separated by $2\varepsilon$, they are still totally ordered, because when projected to the global time, the first event happens in the worst case in the left limit of $t+\varepsilon$ and the second event in its right limit.}
	\end{itemize}
\end{definition}
...
We express atomic propositions as functions of trace values at a time point $T$, i.e., a proposition $p \in \AP$ over a trace $w = (x_1, \ldots, x_n)$ is defined as $f_p(x_1(T), \ldots, x_n(T)) > 0$ where $f_p : \R^n \to \R$ is a function. \dejan{$f$ is already used for the consistent cut flow. I would suggest that we change the symbol for CCF.}
