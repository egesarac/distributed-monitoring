...
For each signal $x_i$, \borzoo{where $1 \leq i \leq n$,} let $F_i$ be the set of end points of its 
uncertainty regions.
...
\end{equation}
\normalsize
\borzoo{Can we define destutter like this too?}The last case happens only when $I \cap R_i$ is 
empty.
...
\begin{example} \label{ex:overapx}
	...
	\borzoo{I would say a signal such as $w$ would be included $\tr^+$.}
