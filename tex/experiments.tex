...
\paragraph{Random Generator Discussion}
...
\paragraph{Heat Pump Discussion.}
...
\paragraph{Swarm of Drones Discussion.}
...
\paragraph{Summary.} To answer RQ1, we observe that while the speedup achieved with ADM compared to EDM is consistenly very high and in three to five orders of maginute, the tradeoff between the efficiency and the accuracy depends to a large extent on the type of specifications, the duration of the input signals and the maximal clock skew. The arithmetic and timed operators are particulatly sensitive to the over-approximations of ADM and reduce the accuracy of the method. Untimed temporal properties, and especially properties in which conjunctive and disjunctive operations are not combined achieve very high levels of accuracy, resulting in an excellent tradeoff. Given the significant increase of the efficiency in ADM compared to EDM, these two methods can be effectively combined even in situations where ADM gives low accuracy. The combination of ADM and EDM results consistently in the overall increase of efficiency, thus giving a positive answer to RQ2.
