...
For sets $L_1, L_2 \in \Sigma^*$ \borzoo{$L_1, L_2 \subseteq \Sigma^*$}of words, we let $L_1 \cdot 
L_2 = \{u \cdot v 
...
\borzoo{citation: orginal STL paper}
Let $A,B \subset \R$.
%
...
\borzoo{citation: our ICCPS'23 paper}
...
In essence, given a distributed signal $(S,{\hb})$, STL$^+$ considers an overapproximation 
$\tr^+(S,{\hb})$ of the set $\tr(S,{\hb})$ of synchronous traces\borzoo{where $ \tr(S,{\hb})  
\subseteq \tr^+(S,{\hb})$ }.
